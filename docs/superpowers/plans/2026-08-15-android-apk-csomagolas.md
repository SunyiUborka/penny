...
- Produces: `getApiBase(): string` a `apps/web/src/api/baseUrl.js`-ből. A
  visszatérési érték sosem végződik `/`-re.

> **Megjegyzés a végleges állapotról.** Ez a task eredetileg egy futásidőben
> felülírható bázis-URL-t szállított (`setApiBase`, `getDefaultApiBase`), mert a
> terv appon belül állítható szervercímet szánt a Task 5-be. A Task 5 review-ja
> után a döntés az lett, hogy a cím fordítási időben fix — a két extra export
> ezért törölve lett. Az alábbi kódrészlet már a végleges állapotot mutatja.
